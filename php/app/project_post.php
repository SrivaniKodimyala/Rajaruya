...
function ajax_post_advertise(){
    ...
    if(isset($_POST['submit'])) {
        ...
        if (empty($_POST['contact_person_name'])) {
            $errors[]['message'] = __("Name of contact person is required");
        }
        
        if (empty($_POST['contact_person_designation'])) {
            $errors[]['message'] = __("Designation of contact person is required");
        }
        
        if (empty($_POST['company_name'])) {
            $errors[]['message'] = __("Name of the Company is required");
        }
        
        if (empty($_POST['client'])) {
            $errors[]['message'] = __("Client is required");
        }
        ...
        // $featured = isset($_POST['make_premium']) ? 1 : 0;

        $package = $_POST['make_premium'];

       // $highlight = isset($_POST['premium']) ? 1 : 0;
        if (!count($errors) > 0) {
            ...
            if ($check_logged_in) {
                ...
                $item_insrt->name_of_contact_person = $_POST['contact_person_name'];
                $item_insrt->desg_of_contact_person = $_POST['contact_person_designation'];
                $item_insrt->name_of_company = $_POST['company_name'];
                $item_insrt->client = $_POST['client'];

                if($package==1){
                    $item_insrt->package='professional'; 
                }elseif($package==2){
                    $item_insrt->package='premium'; 
                }else{
                    $item_insrt->package='basic'; 
                }
