...
if(isset($_POST['action'])){
    if ($_POST['action'] == "save_gig_overview") { save_gig_overview(); }
    if ($_POST['action'] == "edit_gig_overview") { edit_gig_overview(); }
    if ($_POST['action'] == "mark_completed_service") { mark_completed_service(); }
    if ($_POST['action'] == "cancel_order") { cancel_order(); }
...
function save_gig_overview()
{
    ...
    if (!checkloggedin()) {
        $result['status'] = 'error';
        $result['message'] = __("Login with freelancer account.");
        die(json_encode($result));
    }elseif ($_SESSION['user']['user_type'] == 'employer'){
        $result['status'] = 'error';
        $result['message'] = __("Login with freelancer account.");
        die(json_encode($result));
    }
    ...
    if (empty($_POST['catid'])) {
        $errors[]['message'] = __("Category is required");
    }
    if (empty($_POST['subcatid'])) {
        $errors[]['message'] = __("Sub-category is required");
    }
    ...
    if (!isset($_POST['item_screen'])) {
        $errors[]['message'] = __("Image is required.");
    }else{
        if(count($_POST['item_screen']) == 0){
            $errors[]['message'] = __("Image is required.");
        }
    }

    if (empty($_POST['basic_plan_desc'])) {
        $errors[]['message'] = __("Basic plan description is required.");
    }
    if (empty($_POST['standard_plan_desc'])) {
        $errors[]['message'] = __("Standard plan description is required.");
    }
    if (empty($_POST['premium_plan_desc'])) {
        $errors[]['message'] = __("Premium plan description is required.");
    }

    if (empty($_POST['basic_plan_delivery_time'])) {
        $errors[]['message'] = __("Basic plan delivery time is required.");
    }
    if (empty($_POST['standard_plan_delivery_time'])) {
        $errors[]['message'] = __("Standard plan delivery time is required.");
    }
    if (empty($_POST['premium_plan_delivery_time'])) {
        $errors[]['message'] = __("Premium plan delivery time is required.");
    }

    if (empty($_POST['basic_plan_revisions'])) {
        $errors[]['message'] = __("Basic plan revision is required.");
    }
    if (empty($_POST['standard_plan_revisions'])) {
        $errors[]['message'] = __("Standard plan revision is required.");
    }
    if (empty($_POST['premium_plan_revisions'])) {
        $errors[]['message'] = __("Premium plan revision is required.");
    }

    if (empty($_POST['basic_plan_price'])) {
        $errors[]['message'] = __("Basic plan price is required.");
    }
    if (empty($_POST['standard_plan_price'])) {
        $errors[]['message'] = __("Standard plan price is required.");
    }
    if (empty($_POST['premium_plan_price'])) {
        $errors[]['message'] = __("Premium plan price is required.");
    }

    if (!count($errors) > 0) {
        $status = ($config['post_auto_approve'] == 1)? "active" : "pending";
        $title = validate_input($_POST['title']);
        $desc = validate_input($_POST['content'],true);
        $cat_id = validate_input($_POST['catid']);
        $sub_catid = validate_input($_POST['subcatid']);
        $price = validate_input($_POST['basic_plan_price']);
        $tags = isset($_POST['tags'])? validate_input($_POST['tags']) : '';
        $slug = create_slug($title);
        $save_post = ORM::for_table($config['db']['pre'].'post')->create();
        $save_post->user_id = $_SESSION['user']['id'];
        $save_post->status = $status;
        $save_post->post_type = 'gig';
        $save_post->slug = $slug;
        $save_post->title = $title;
        $save_post->description = $desc;
        $save_post->category = $cat_id;
        $save_post->sub_category = $sub_catid;
        $save_post->price = $price;
        $save_post->tag = $tags;
        $save_post->save();

        $post_id = $save_post->id();
        add_post_customField_data($cat_id, $sub_catid, $post_id,'gig');

        $post_link = $link['SERVICE'] . "/" . $post_id. "/" .$slug;
        /*Here we are saving post options*/
        $item_screen = "";
        if (isset($_POST['item_screen']) && count($_POST['item_screen']) > 0) {
            ...
            foreach ($_POST['item_screen'] as $name) {
                ...
                if (!empty($filename)) {
                    ...
                    if (in_array($ext, $valid_formats)) {
                        ...
                        $errors[]['message'] = __("Sorry, only JPG files are allowed.");
                    }
                    if ($countScreen == 0)
            ...
            add_post_option($post_id,'images',$item_screen);
        }
        /*Here we are saving post options*/

        /*Here we are saving pricing plan options*/
        $basic_plan_title = isset($_POST['basic_plan_title'])? validate_input($_POST['basic_plan_title']) : __('Basic');
        $standard_plan_title = isset($_POST['standard_plan_title'])? validate_input($_POST['standard_plan_title']) : __('Standard');
        $premium_plan_title = isset($_POST['premium_plan_title'])? validate_input($_POST['premium_plan_title']) : __('Premium');

        $basic_plan_desc = validate_input($_POST['basic_plan_desc']);
        $standard_plan_desc = validate_input($_POST['standard_plan_desc']);
        $premium_plan_desc = validate_input($_POST['premium_plan_desc']);

        $basic_plan_delivery_time = validate_input($_POST['basic_plan_delivery_time']);
        $standard_plan_delivery_time = validate_input($_POST['standard_plan_delivery_time']);
        $premium_plan_delivery_time = validate_input($_POST['premium_plan_delivery_time']);

        $basic_plan_revisions = validate_input($_POST['basic_plan_revisions']);
        $standard_plan_revisions = validate_input($_POST['standard_plan_revisions']);
        $premium_plan_revisions = validate_input($_POST['premium_plan_revisions']);

        $basic_plan_price = validate_input($_POST['basic_plan_price']);
        $standard_plan_price = validate_input($_POST['standard_plan_price']);
        $premium_plan_price = validate_input($_POST['premium_plan_price']);

        $basic_price_plan = json_encode(array(
            'id' => 'basic',
            'name' => $basic_plan_title,
            'description' => $basic_plan_desc,
            'delivery_time' => $basic_plan_delivery_time,
            'revisions' => $basic_plan_revisions,
            'price' => $basic_plan_price
        ));
        add_post_option($post_id,'basic_pricing_plan', $basic_price_plan);

        $standard_price_plan = json_encode(array(
            'id' => 'standard',
            'name' => $standard_plan_title,
            'description' => $standard_plan_desc,
            'delivery_time' => $standard_plan_delivery_time,
            'revisions' => $standard_plan_revisions,
            'price' => $standard_plan_price
        ));
        add_post_option($post_id,'standard_pricing_plan', $standard_price_plan);

        $premium_price_plan = json_encode(array(
            'id' => 'premium',
            'name' => $premium_plan_title,
            'description' => $premium_plan_desc,
            'delivery_time' => $premium_plan_delivery_time,
            'revisions' => $premium_plan_revisions,
            'price' => $premium_plan_price
        ));
        add_post_option($post_id,'premium_pricing_plan', $premium_price_plan);

        /*Here we are saving pricing plan options*/
        $result['success'] = true;
        $result['status'] = 'success';
        $result['redirect'] = $post_link;
        $result['message'] = __("Overview Saved.");
        die(json_encode($result));
    }
...
function edit_gig_overview()
{
    ...
    if (!checkloggedin()) {
        ...
        $result['message'] = __("Login with freelancer account.");
        die(json_encode($result));
    }elseif(!check_valid_seller($post_id)){
        $result['status'] = 'error';
        $result['message'] = __("This gig service not belongs to you..");
        die(json_encode($result));
    }
    ...
    if (!count($errors) > 0) {
        ...
        $update_post = ORM::for_table($config['db']['pre'].'post')->find_one($post_id);
        $update_post->title = $title;
        $update_post->description = $desc;
        $update_post->category = $cat_id;
        $update_post->sub_category = $sub_catid;
        $update_post->price = $price;
        $update_post->tag = $tags;
        $update_post->save();

        add_post_customField_data($cat_id, $sub_catid, $post_id,'gig');

        $post_link = $link['SERVICE'] . "/" . $post_id. "/" .create_slug($title);
        /*Here we are saving post options*/
        $item_screen = "";
        if (isset($_POST['item_screen']) && count($_POST['item_screen']) > 0) {
            $valid_formats = array("jpg", "jpeg", "png"); // Valid image formats
            ...
            update_post_option($post_id,'images',$item_screen);
        }
        /*Here we are saving post options*/
        ...
        update_post_option($post_id,'basic_pricing_plan', $basic_price_plan);
        ...
        update_post_option($post_id,'standard_pricing_plan', $standard_price_plan);
        ...
        update_post_option($post_id,'premium_pricing_plan', $premium_price_plan);

        /*Here we are saving pricing plan options*/
        $result['success'] = true;
        $result['status'] = 'success';
        $result['redirect'] = $post_link;
        $result['message'] = __("Overview Updated.");
        die(json_encode($result));
    }
    $json = '{"status" : "' . $status . '","errors" : ' . json_encode($errors, JSON_UNESCAPED_SLASHES) . '}';
    echo $json;
    ...
    if($valid_check_stamp){
        ...
        if($post_type == "gig"){
            ...
            $order = ORM::for_table($config['db']['pre'] . 'orders')
                ->table_alias('o')
                ->select_many_expr('o.id order_id', 'o.post_id', 'o.user_id', 'p.user_id freelancer_id')
                ->where($where_array)
                ->join($config['db']['pre'] . 'post', array('o.post_id', '=', 'p.id'), 'p')
                ->find_one($order_id);

            $post_id = $order['post_id'];
            $project_title = "#".$order['order_id'];
            $freelancer_id = $order['freelancer_id'];
            $employer_id = $order['user_id'];

            $save_reviews = ORM::for_table($config['db']['pre'].'reviews')->create();
            $save_reviews->post_type = $post_type;
            $save_reviews->project_id = $post_id;
            $save_reviews->order_id = $order_id;
...
function create_milestone(){
    ...
    if(isset($_POST['id']))
    {
        $proj_id = (int) validate_input($_POST['id']);
        $amountToBePay=ORM::for_table($config['db']['pre'] . 'milestone')
        ...
        if ($amountToBePay == 0) {
            ...
            $twentyPercent = $_POST["totalProjectAmount"] * 0.2;
         $enteredAmount = floatval($_POST["amount"]); 
  
         if ($enteredAmount != $twentyPercent) {
           
      $result['success'] = false;
     $result['message'] = __("You can only pay up to 20% of the total project amount for the first milestone.");
     die(json_encode($result));
            }
    }  elseif ($amountToBePay == 1) {
        $twentyPercent = $_POST["totalProjectAmount"] * 0.6;
     $enteredAmount = floatval($_POST["amount"]); 
     if ($enteredAmount !=  $twentyPercent) {
  $result['success'] = false;
 $result['message'] = __("You can only pay up to 60% of the total project amount for the second milestone..");
 die(json_encode($result));      
}
} elseif ($amountToBePay == 2) {
    $twentyPercent = $_POST["totalProjectAmount"] * 0.2;
 $enteredAmount = floatval($_POST["amount"]); 
 if ($enteredAmount != $twentyPercent) {
$result['success'] = false;
$result['message'] = __("You can only pay up to 20% of the total project amount for the third milestone..");
die(json_encode($result));    
}
}
    }
    ...
    if(isset($_POST['id']))
    {
        ...
        if($_SESSION['user']['user_type'] == 'employer'){
            $project = ORM::for_table($config['db']['pre'] . 'project')
                ...
                $step = ORM::for_table($config['db']['pre'] . 'milestone')
                ->select_many('step')
...
$stepVal=$step['step'];
        ...
        if($_SESSION['user']['user_type'] == 'employer'){
            ...
            $total_project_amount = validate_input($_POST['totalProjectAmount']);
            $employer_commission = $group_info['employer_commission'];

            if($employer_commission != 0 && $stepVal != 1){
                $comission = (($total_project_amount/100)*$employer_commission);
            }else{
                $comission = 0;
            }
            ...
            {
                $result['success'] = false;
                $result['message'] = __("Wallet balance must be greater than").' '.$config['currency_sign'].$t_amount.'.';
                die(json_encode($result));
            }
            else
            {
...
if(empty($step)){
    ...
    $stepValForStepOne=1;
                ...
                $create_milestone->step = $stepValForStepOne;
                $create_milestone->total_amount = $total_project_amount;
...
function release_milestone(){
    ...
    if(isset($_POST['id']) && checkloggedin() && $_SESSION['user']['user_type'] == 'employer') {
        ...
        if (!empty($milestone)) {
          ...
          $step=$milestone['step'];
            $total_project_amt=$milestone['total_amount'];
            $group_info_emp = get_user_membership_settings();
            $employer_commission_emp = $group_info_emp['employer_commission'];
            if($step==1)
            {
                if($employer_commission_emp != 0){
                    $comission_emp = (($total_project_amt/100)*$employer_commission_emp);
                    $amount=($amount-$comission_emp);
                } 
            }
...
function mark_completed_service(){
    global $config,$lang,$link;
    ...
    if(isset($_POST['id']) && $_SESSION['user']['user_type'] == 'employer')
    {
        $order_id = validate_input($_POST['id']);
        $count = ORM::for_table($config['db']['pre'] . 'orders')
            ->table_alias('o')
            ->select_many_expr('o.*', 'p.id post_id', 'p.user_id seller_id', 'p.title post_title')
            ->where(array(
                'o.id' => $order_id,
                'o.user_id' => $_SESSION['user']['id'],
                'o.status' => 'progress'
        ...
        if ($count) {
            ...
            $group_info = get_user_membership_settings();
            $service_commission = $group_info['service_seller_commission'];
            if($service_commission != 0){
                $comission = (($order_amount/100)*$service_commission);
                minus_balance($seller_id,$comission);
            }
            ...
            if($comission > 0){
                ...
                $updated_amount = ($current_amount - $order_amount + $comission);
                $total_earning =  ($total_earning - $order_amount + $comission);
            ...
            $update_balance = ORM::for_table($config['db']['pre'].'user')
                ->where('id' , $seller_id)
                ->find_one();
            $update_balance->set_expr('balance', 'balance+'.$order_amount);
            $update_balance->save();

            $now = time();
            $ip = encode_ip($_SERVER, $_ENV);
            $trans_insert = ORM::for_table($config['db']['pre'].'transaction')->create();
            $trans_insert->product_name = 'Order completed';
            $trans_insert->product_id = $order_id;
            $trans_insert->user_id = $seller_id;
            $trans_insert->status = 'success';
            $trans_insert->amount = $order_amount;
            $trans_insert->transaction_gatway = 'Wallet';
            $trans_insert->transaction_ip = $ip;
            $trans_insert->transaction_time = $now;
            $trans_insert->transaction_description = 'Order completed';
            $trans_insert->transaction_method = 'order_completed';
            $trans_insert->save();
            ...
            /*EMAIL-Template here when order completed by employer email to freelancer*/

            /*EMAIL-10: Freelancer : Project Awarded*/

            $result['success'] = true;
            $result['message'] = __("Order completed. Redirecting...");

        }else {
            $result['success'] = false;
            $result['message'] = __("Order does not belong to you");
        }
    }else {
        $result['success'] = false;
        $result['message'] = __("Error: Please try again.");
...
function cancel_order(){
    ...
    if(isset($_POST['id']) && $_SESSION['user']['user_type'] == 'user')
    {
        $order_id = validate_input($_POST['id']);
        $cancel_reason = validate_input($_POST['reason']);
        ...
        if($count){
            ...
            $result['success'] = true;
            $result['message'] = __("Order has been cancelled.");
        }else{
            $result['success'] = false;
            $result['message'] = __("Order does not belong to you");
        }
...
function searchCityFromCountry()
{
    ...
    $sql = "SELECT c.id, c.asciiname, c.latitude, c.longitude, c.subadmin1_code, s.name AS statename, co.asciiname AS countryname
...
function getCustomFieldByCatID()
{
    ...
    $post_type = isset($_POST['post_type']) ? $_POST['post_type'] : 'default';

    if ($maincatid > 0) {
        $custom_fields = get_customFields_by_catid($post_type, $maincatid,$subcatid);
