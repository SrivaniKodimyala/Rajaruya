...
<div class="small-footer margin-top-15 dark-modeforbordertop">
    <div class="small-footer-copyrights dark-modetextgray">
       <?php
   
   $copyright_text = _esc($config['copyright_text'], false);

 
   $pipe_position = strpos($copyright_text, '|');


   if ($pipe_position !== false) {

       $text_after_pipe = substr($copyright_text, $pipe_position + 1);

       $text_after_pipe = trim($text_after_pipe);


       $replacement = '<a href="https://ultrakeyit.com/" target="_blank" style="color: #20C997;">' . $text_after_pipe . '</a>';


       $modified_copyright_text = substr($copyright_text, 0, $pipe_position + 1) . ' ' . $replacement;
   } else {
   
       $modified_copyright_text = $copyright_text;
   }

   // Output the modified text
   echo $modified_copyright_text;
   ?>
    </div>
    <ul class="footer-social-links">
        <?php
        if($config['facebook_link'] != "")
            echo '<li><a href="'._esc($config['facebook_link'],false).'" target="_blank" rel="nofollow"><i class="fa fa-facebook dark-modetextgray"></i></a></li>';
        if($config['twitter_link'] != "")
            echo '<li><a href="'._esc($config['twitter_link'],false).'" target="_blank" rel="nofollow"><i class="fa fa-twitter dark-modetextgray"></i></a></li>';
        if($config['instagram_link'] != "")
            echo '<li><a href="'._esc($config['instagram_link'],false).'" target="_blank" rel="nofollow"><i class="fa fa-instagram dark-modetextgray"></i></a></li>';
        if($config['linkedin_link'] != "")
            echo '<li><a href="'._esc($config['linkedin_link'],false).'" target="_blank" rel="nofollow"><i class="fa fa-linkedin dark-modetextgray"></i></a></li>';
        if($config['pinterest_link'] != "")
            echo '<li><a href="'._esc($config['pinterest_link'],false).'" target="_blank" rel="nofollow"><i class="fa fa-pinterest dark-modetextgray"></i></a></li>';
        if($config['youtube_link'] != "")
            echo '<li><a href="'._esc($config['youtube_link'],false).'" target="_blank" rel="nofollow"><i class="fa fa-youtube dark-modetextgray"></i></a></li>';
        ?>
    </ul>
