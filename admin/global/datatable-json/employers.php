...
$where = $sqlTot = $sqlRec = "";
if (!empty($where)) {
    $where .= " AND user_type = 'employer'";
} else {
    $where = " WHERE user_type = 'employer'";
}
...
$upgrades_title='Basic';
    ...
    $row1 = '<td class="text-center">
            ...
            $rowMembershiplan = '<td class="hidden-xs">'.$upgrades_title.'</td>';
            $rowStart = '<td class="hidden-xs">'.$upgrades_start_date.'</td>';
            $rowExpiry = '<td class="hidden-xs">'.$upgrades_expiry_date.'</td>';
    ...
    $row7 = '<td class="text-center">
                <div class="btn-group">
                    <a href="#" class="btn btn-xs btn-default login-as-user" data-user-id="'.$id.'">Login as user</a>
