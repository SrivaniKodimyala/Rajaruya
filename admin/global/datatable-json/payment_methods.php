...
foreach ($queryRecords as $row) {
    ...
    if($row['payment_install'] == 1)
        $install = '<span class="label label-info">Enabled</span>';
    else
        $install = '<span class="label label-warning">Disabled</span>';

    if($row['payment_install'] == 1) {

        $install_button = '<a href="#" class="btn btn-xs btn-warning uninstall-payment" data-ajax-action="uninstallPayment" data-folder="'.$folder.'"><i class="fa fa-close"></i> Disable</a>';
    }
    else{
        $install_button = '<a href="#" class="btn btn-xs btn-success install-payment" data-ajax-action="installPayment" data-folder="'.$folder.'"><i class="fa fa-download"></i> Enable</a>';
    }
