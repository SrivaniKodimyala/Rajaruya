...
{
    $file_name = validate_input($_POST['file_name']);
...
function installPayment()
{
    ...
    $folder = validate_input($_POST['folder']);
    if (trim($id) != '') {
        if(check_allow()) {
    ...
    if (trim($id) != '') {
        if(check_allow()) {
            ...
            if($row > 0){
                ...
                $product_update->set('custom_fields', validate_input($info['custom_fields']));
                $product_update->set('custom_types', validate_input($info['custom_types']));
                $product_update->set('custom_values', validate_input($info['custom_values']));
...
function addNewCat()
{
    ...
    $picture = validate_input($_POST['picture']);
    if (trim($name) != '' && is_string($name)) {
        if($slug == "")
...
function quickad_update_custom_option_position()
{
    ...
    if (is_array($position)) {
        ...
        $custom_id = validate_input($_POST['field_id']);
...
function edit_langTranslation_PostTypes()
{
    ...
    $id = validate_input($_POST['id']);
    $trans_lang = implode(',', validate_input($_POST['trans_lang']));
    //$trans_name = implode(',', $_POST['trans_name']);
    $trans_name = validate_input($_POST['trans_name']);
    /*$i = 0;
...
function delete_custom_option(){
    ...
    if(isset($_POST['id'])){
        $id = validate_input($_POST['id']);
        $field_id = validate_input($_POST['field_id']);
    }
    ...
    foreach($fields as $custom) {
        ...
        if(check_allow()){
            ...
            if($exist > 0){
                ...
                $query = "INSERT INTO `" . $config['db']['pre'] . "custom_fields` SET post_type = '$post_type',translation_lang = '$trans_lang', translation_name = '$trans_name', `custom_anycat` = '".$allcat."',`custom_catid` = '".$maincat."',`custom_subcatid` = '".$category."',`custom_title` = '".$title."', `custom_type` = '".$type."',`custom_required` = '".$required."',`custom_options` = '".$options."' ";
                $con->query($query) OR error(mysqli_error($con));
...
function getDistrictSelectedforCityAdd()
{
    ...
    $code = isset($_POST['id']) ? validate_input($_POST['id']) : 0;
    $selectid = isset($_POST['selectid']) ? validate_input($_POST['selectid']) : "";
...
function searchCityStateCountry()
{
    ...
    $dataString = isset($_POST['dataString']) ? validate_input($_POST['dataString']) : "";
...
function getlocHomemap()
{
    ...
    if(isset($_GET['serachStr'])){
        $serachStr = validate_input($_GET['serachStr']);
    }
    else{
        $serachStr = '';
    }
...
function openlocatoionPopup()
{
    ...
    $query = "SELECT * FROM `".$config['db']['pre']."product` WHERE id = '" . validate_input($_POST['id']) . "' LIMIT 1";
    $query_result = mysqli_query ($con, $query);
    $data = array();
    $i = 0;
    if ($query_result->num_rows > 0) {
