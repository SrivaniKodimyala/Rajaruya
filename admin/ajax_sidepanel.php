    ...
    if (isset($_POST['id'])) {
        ...
        if (isset($_POST['status'])) {
            ...
            if($_POST['status'] == "reject"){
                ...
                $amount = $info['amount'];
                add_balance($user_id,$amount,"Withdrawal Rejected");
...
function paymentEdit()
{
    ...
    if (isset($_POST['id'])) {
        ...
        if(isset($_POST['paypal_sandbox_mode'])){
            ...
            update_option("paypal_api_client_id",isset($_POST['paypal_api_client_id'])? trim($_POST['paypal_api_client_id']) : "");
            update_option("paypal_api_secret",isset($_POST['paypal_api_secret'])? trim($_POST['paypal_api_secret']) : "");
        }

        if(isset($_POST['stripe_secret_key'])){
            update_option("stripe_payment_mode",validate_input($_POST['stripe_payment_mode']));
            update_option("stripe_publishable_key",validate_input(trim($_POST['stripe_publishable_key'])));
            update_option("stripe_secret_key",validate_input(trim($_POST['stripe_secret_key'])));
            ...
            if(empty($_POST['stripe_webhook_secret'])){
                ...
                try{
                    if(file_exists('../includes/payments/stripe/stripe-php/init.php')) {
                        ...
                        $stripe = new \Stripe\StripeClient(validate_input(trim($_POST['stripe_secret_key'])));
                ...
                update_option("stripe_webhook_secret", trim($_POST['stripe_webhook_secret']));
            }
        }

        if(isset($_POST['paystack_public_key'])){
            update_option("paystack_public_key",validate_input(trim($_POST['paystack_public_key'])));
            update_option("paystack_secret_key",validate_input(trim($_POST['paystack_secret_key'])));
        }

        if(isset($_POST['payumoney_merchant_key'])){
            update_option("payumoney_sandbox_mode",validate_input($_POST['payumoney_sandbox_mode']));
            update_option("payumoney_merchant_key",validate_input(trim($_POST['payumoney_merchant_key'])));
            update_option("payumoney_merchant_salt",validate_input(trim($_POST['payumoney_merchant_salt'])));
            update_option("payumoney_merchant_id",validate_input(trim($_POST['payumoney_merchant_id'])));
        }
        ...
        if(isset($_POST['PAYTM_ENVIRONMENT'])){
            ...
            update_option("PAYTM_MERCHANT_KEY",validate_input(trim($_POST['PAYTM_MERCHANT_KEY'])));
            update_option("PAYTM_MERCHANT_MID",validate_input(trim($_POST['PAYTM_MERCHANT_MID'])));
            update_option("PAYTM_MERCHANT_WEBSITE",validate_input($_POST['PAYTM_MERCHANT_WEBSITE']));
        }

        if(isset($_POST['mollie_api_key'])){
            update_option("mollie_api_key",validate_input(trim($_POST['mollie_api_key'])));
        }

        if(isset($_POST['iyzico_api_key'])){
            update_option("iyzico_sandbox_mode",validate_input($_POST['iyzico_sandbox_mode']));
            update_option("iyzico_api_key",validate_input(trim($_POST['iyzico_api_key'])));
            update_option("iyzico_secret_key",validate_input(trim($_POST['iyzico_secret_key'])));
        }

        if(isset($_POST['midtrans_client_key'])){
            update_option("midtrans_sandbox_mode",validate_input($_POST['midtrans_sandbox_mode']));
            update_option("midtrans_client_key",validate_input(trim($_POST['midtrans_client_key'])));
            update_option("midtrans_server_key",validate_input(trim($_POST['midtrans_server_key'])));
        }

        if(isset($_POST['paytabs_profile_id'])){
            update_option("paytabs_profile_id",validate_input(trim($_POST['paytabs_profile_id'])));
            update_option("paytabs_secret_key",validate_input(trim($_POST['paytabs_secret_key'])));
        }

        if(isset($_POST['telr_store_id'])){
            update_option("telr_sandbox_mode",validate_input(trim($_POST['telr_sandbox_mode'])));
            update_option("telr_store_id",validate_input(trim($_POST['telr_store_id'])));
            update_option("telr_authkey",validate_input(trim($_POST['telr_authkey'])));
        }

        if(isset($_POST['razorpay_api_key'])){
            update_option("razorpay_api_key",validate_input(trim($_POST['razorpay_api_key'])));
            update_option("razorpay_secret_key",validate_input(trim($_POST['razorpay_secret_key'])));
        }

        if(isset($_POST['phonepe_secret_key'])){
            update_option("phonepe_shop_id",validate_input(trim($_POST['phonepe_shop_id'])));
            update_option("phonepe_secret_key",validate_input(trim($_POST['phonepe_secret_key'])));
        }

        if(isset($_POST['yoomoney_secret_key'])){
            update_option("yoomoney_shop_id",validate_input(trim($_POST['yoomoney_shop_id'])));
            update_option("yoomoney_secret_key",validate_input(trim($_POST['yoomoney_secret_key'])));
        }
...
function SaveSettings(){
    ...
    if (isset($_POST['social_login_setting'])) {
        update_option("facebook_app_id",validate_input(trim($_POST['facebook_app_id'])));
        update_option("facebook_app_secret",validate_input(trim($_POST['facebook_app_secret'])));
        update_option("google_app_id",validate_input(trim($_POST['google_app_id'])));
        update_option("google_app_secret",validate_input(trim($_POST['google_app_secret'])));
        $status = "success";
        $message = __("Saved Successfully");
    }
