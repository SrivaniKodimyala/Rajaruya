...
//   die();
    ...
    $post_type = "default";
    $resultForService = ORM::for_table($config['db']['pre'].'catagory_main')
        ...
        // die();
        foreach ($resultForService as $infoService) {
            $categoryService[$infoService['cat_id']]['slug'] = $infoService['slug'];
            $categoryService[$infoService['cat_id']]['name'] = $infoService['cat_name'];
            if(trim($config['home_page']) == "home-freelance"){
                $categoryService[$infoService['cat_id']]['link'] = $link['SEARCH_PROJECTS']."/".$infoService['slug'];
            }elseif (trim($config['home_page']) == "home-job"){
                $categoryService[$infoService['cat_id']]['link'] = $link['SEARCH_CAT']."/".$infoService['slug'];
            } else{
                $categoryService[$infoService['cat_id']]['link'] = $link['SEARCH_SERVICES']."/".$infoService['slug'];
            }
        }



    $resultProjects = ORM::for_table($config['db']['pre'].'catagory_main')
    ->where("post_type","default")
    ->order_by_asc('cat_order')
    ->limit(8)
    ->find_many();
  
foreach ($resultProjects as $infoProjects) {
if($config['lang_code'] != 'en' && $config['userlangsel'] == '1'){
    $maincatProjects = get_category_translation("main",$infoProjects['cat_id']);
    if(is_array($maincatProjects) && count($maincatProjects) > 0) {
        $infoProjects['cat_name'] = $maincatProjects['title'];
        $infoProjects['slug'] = $maincatProjects['slug'];
    }
}
$categoryProjects[$infoProjects['cat_id']]['slug'] = $infoProjects['slug'];
$categoryProjects[$infoProjects['cat_id']]['icon'] = $infoProjects['icon'];
$categoryProjects[$infoProjects['cat_id']]['name'] = $infoProjects['cat_name'];
$categoryProjects[$infoProjects['cat_id']]['main_id'] = $infoProjects['cat_id'];
$categoryProjects[$infoProjects['cat_id']]['picture'] = $infoProjects['picture'];


$categoryProjects[$infoProjects['cat_id']]['link'] = $link['SEARCH_SERVICES']."/".$infoProjects['slug'];

$totalAdsMaincatGig = ORM::for_table($config['db']['pre'].'post')
->where(array(
    'category'=> $infoProjects['cat_id'],
    'status'=> 'active'
))
->count();


$categoryProjects[$infoProjects['cat_id']]['main_ads_count'] = $totalAdsMaincatGig;
$countGig = 1;
...
function payment_success_save_detail($access_token){
    ...
    elseif($payment_type == "membershipsubscr"){
        ...
        // Check that the payment is valid
         ...
         if($plan_interval == 'YEARLY') {
            ...
            $sub_group_id =  $subcription_id;

            $subsc_check = ORM::for_table($config['db']['pre'].'membershipupgrades')
                ->where('user_id', $user_id)
                ->count();
            if($subsc_check == 1)
            {
            ...
            // Add time to their subscription
            $expires = (time()+$term);
  $upgrade_lasttime = time(); // Record the current time as the last upgrade time
            if($txn_type == 'subscr_update')
            {

               $query = "UPDATE `".$config['db']['pre']."membershipupgrades` SET `sub_id` = '".validate_input($subcription_id)."',`upgrade_lasttime` = '".validate_input($upgrade_lasttime)."' ,`upgrade_expires` = '".validate_input($expires)."' WHERE `user_id` = '".validate_input($user_id)."' LIMIT 1 ";
                $pdo->query($query);

                $person = ORM::for_table($config['db']['pre'].'user')->find_one($user_id);
                $person->membershipplan_id = $sub_group_id;
                $person->save();
            ...
            elseif($txn_type == 'subscr_signup')
            {
                ...
                $upgrades_insert = ORM::for_table($config['db']['pre'].'membershipupgrades')->create();
                $upgrades_insert->sub_id = $subcription_id;
                $upgrades_insert->user_id = $user_id;
                $upgrades_insert->upgrade_lasttime = $now;
                $upgrades_insert->upgrade_expires = $expires;
