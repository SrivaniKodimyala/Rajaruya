...
function email_template($template, $user_id=null, $password=null, $product_id=null, $item_title=null){
    ...
    if ($template == "inquiry") {
        ...
        // Add main category and subcategory information
        $mainCategory = $_POST['main_category']; // Assuming 'main_category' is the name attribute of your main category select input
        $subcategory = $_POST['sub_category'];   // Assuming 'sub_category' is the name attribute of your subcategory select input
        // $mainCategoryName = fetchMainCategoryName($mainCategory);
        // Append main category and subcategory to the email subject and body
        $html = str_replace('{MAIN_CATEGORY}', $mainCategory, $html);
        $html = str_replace('{SUB_CATEGORY}', $subcategory, $html);
        
        $email_subject = $html;

        $html = $config['inquiry_message_contact'];
        $html = str_replace('{SITE_TITLE}', $config['site_title'], $html);
        $html = str_replace('{SITE_URL}', $config['site_url'], $html);
        $html = str_replace('{NAME}', $_POST['name'], $html);
        $html = str_replace('{EMAIL}', $_POST['email'], $html);
